    ...
    <div class="container w-full flex gap-[117px] <?php if ($cols_reverse) echo 'flex-row-reverse'; ?> ">
        <div class="col1 basis-1/2 flex-col">
            <?php if ($section_number) : ?>
                <p class="w-fit text-[16px] leading-[24px] text-[#60B8D1] font-semibold after:content-[''] after:w-[61px] after:h-[2px] after:bg-secondary after:rounded-full after:absolute after:top-1/2 after:-translate-y-1/2 after:right-[-70px] relative"><?php echo $section_number; ?></p>
            <?php endif; ?>

            <?php if ($title) : ?>
                <p class="text-[60px] text-[#0F304D] leading-[72px] font-medium"><?php echo $title; ?></p>
            <?php endif; ?>
