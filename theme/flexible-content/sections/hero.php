...
<section class="relative w-full !h-[600px] md:!h-[700px] flex flex-col justify-center">
    <picture class="absolute inset-0 -z-10 object-cover !h-full w-full">
        ...
        <img src="<?= $hero_bg_url_mobile ?>" class="w-full h-full" <?= $hero_background_mobile['alt'] ? 'alt="'.$hero_background_mobile['alt'] . '"' : '' ?> >
    </picture>
    ...
    <div class="container">
        <div class="relative mb-10 z-0 flex flex-col justify-center">
            <?php if ($small_title) : ?>
                <h2 class="w-fit text-base leading-[24px] font-semibold text-white after:content-[''] after:w-[61px] after:h-[2px] after:bg-white after:rounded-full after:absolute after:top-1/2 after:-translate-y-1/2 after:right-[-70px] relative"><?php echo $small_title; ?></h2>
            <?php endif; ?>
            <?php if ($hero_text) : ?>
