...
function renderJobList(jobs, container) {
	jobs.forEach((job, index) => {
	  const jobHtml = createJobHtml(job, index + 1, job.isFeatured);
	  container.insertAdjacentHTML('beforeend', jobHtml);
	});
}

function extractJobListData(rawListContainer) {
	const jobs = Array.from(rawListContainer.querySelectorAll('[skkresult="offer"]')).map((jobEl,index) => {
		...
		const featuredArray = rawListContainer.dataset.jobsFeatured.split(',').map(Number);
		const currJobNumber = (currentPage - 1) * jobsPerPage + index + 1
		const isFeatured = featuredArray.includes(currJobNumber)
...
function createJobHtml(jobData, index, isFeatured = false) {
	...
	return `
		<a href="${jobData.url}" target="_blank" class="animate-appear group py-8 md:py-10 flex flex-col lg:flex-row justify-between border-b-[1px] border-b-[#BEBFBF]" style="animation-delay: ${index * 100}ms;">
			<div class="flex gap-[26px]">
				<span class="text-[30px] w-9 font-medium ${isFeatured ? 'text-red-500' : 'text-secondary'}">${formattedNumber}</span>
				<div class="mb-5 lg:mb-0">
					<h3 class="mb-[10px] text-[30px] font-medium ${isFeatured ? 'text-red-500' : 'text-primary'} group-hover:text-secondary transition duration-200">${jobData.title}</h3>
